...
class Submittor extends Thread {
   ...
   * string will vary depending on the type of com.egurnee.school.os.p3.sample we are implementing.  For every type of com.egurnee.school.os.p3.sample
   * the first token will be the name of a job/process and the second token will be an integer representing
   * the number of msecs before this job should be submitted to the OS.  For a FCFS com.egurnee.school.os.p3.sample the remainder 
  ...
  public void run() {
    ...
    for (String jobDesc : myJobDescs) {
    	...
    	int delay = 0; // msec delay until this Job is submitted to the kernel
    	String burstDescription = null;  // The description of that Job.  (For FCFS this will be a single integer token)
